...
func main() {
	...
	html, err := ioutil.ReadFile(path.Join(os.Getenv("GOPATH"), "src/github.com/goclub/session/internal/example/app/index.html")) ; HandleError(nil, err)
	http.HandleFunc("/login", func(writer http.ResponseWriter, request *http.Request) {
		ctx := request.Context()
		sessionID, err := sessHub.NewSessionID(ctx) ; HandleError(writer, err)
	...
	http.HandleFunc("/", func(writer http.ResponseWriter, request *http.Request) {
		...
		session, sessionExpired, err := sessHub.GetSessionBySessionID(ctx, sessionID) ; HandleError(writer, err)
		if sessionExpired {
